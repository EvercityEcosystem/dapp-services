...
import Form from "@/components/forms/AgridataxForm";
...
export default {
  ...
  data() {
    return {
      ...
      allowance: 0,
      demandId: 0,
      model: config.ROBONOMICS.model.agridatax
    };
  },
  components: {
    Page,
  ...
  created() {
    document.title = this.$t("agridatax.title") + " | " + this.$t("title");

    const configChain = config.chain.get();
    this.token = configChain.token.agridatax;
    this.validator = configChain.validator.agridatax;
  },
  methods: {
    submit() {
