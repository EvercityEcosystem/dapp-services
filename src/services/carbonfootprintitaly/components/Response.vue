<template>
  <div class="t-align--center">
    <div>{{ $t("carbonfootprintitaly.priceTitle") }}:</div>
    <b class="t-gt">{{ cost | fromWei(decimals, symbol) }}</b>
    <p>
      <a href="javascript:;" @click="details = !details">
        <template v-if="details">{{
          $t("carbonfootprintitaly.detailsLess")
        }}</template>
        <template v-else>{{ $t("carbonfootprintitaly.details") }}</template>
      </a>
    </p>
    <div v-show="details" style="max-width: 600px; margin: 10px auto;">
      <div class="row">
        <div class="col-sm-6">
          <div class="m-b-5">
            <span>{{ $t("carbonfootprintitaly.agent") }}:</span>&nbsp;
            <RChainExplorer :address="sender" />
          </div>
          <div class="m-b-5">
            <span>{{ $t("carbonfootprintitaly.token") }}:</span>&nbsp;
            <RChainExplorer :address="address" category="token" />
          </div>
          <div class="m-b-5">
            <span>{{ $t("carbonfootprintitaly.objective") }}:</span>&nbsp;
